...
### 単位と表示
- **気圧**: システム全体で `hPa` (ヘクトパスカル) 単位で統一されています。
  - DB保存: `hPa`
  - API返却: `hPa`
  - UI表示: 整数値（四捨五入）として表示。色は紫色 (`#9b59b6`)。
- **温度**: `°C` 単位。UIでは小数点第1位まで表示。色は緑色 (`#2ecc71`)。
- **湿度**: `%` 単位。整数値として表示。色は青色 (`#3498db`)。

### データの整合性
過去に `Pa` 単位（101300等）で保存されていたデータがある場合は、以下のスクリプトで一括変換が可能です。
```bash
